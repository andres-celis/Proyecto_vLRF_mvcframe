...
class create 
{
    ...
    public function __construct(){//Contador de llamados.
       
        $this->contador=0; 
    }
    
    
    public function definir($j,$valor)
      {
            
              
              if (array_key_exists("value", $valor[$j])) {
    
    //Retonar el valor del nodo especificado por j;
    
                  return $valor[$j]["value"];
              } else {
                  
                  if (array_key_exists("RDF:RESOURCE", $valor[$j]["attributes"])) { //Retonar el RDF:RESOURCE del nodo especificado por j;
                      return $valor[$j]["attributes"]["RDF:RESOURCE"];
                  }
                  if (array_key_exists("RDF:DATATYPE", $valor[$j]["attributes"])) {//Retonar el RDF:DATATYPE del nodo especificado por j;
                      return $valor[$j]["attributes"]["RDF:DATATYPE"];
                  }
              }
          }
